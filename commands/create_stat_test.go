	...
	Context("#CreateStat", func() {
		...
		Context("When a valid date is provided in timestamp format", func() {
			...
			It("Does not return an error", func() {
				err := commands.CreateStat("a-resource", "key1|key2", "1521282189000", 75, &fakeClient)
		...
		Context("When a valid date is provided in RFC3399 format", func() {
			...
			It("Does not return an error", func() {
				err := commands.CreateStat("a-resource", "key1|key2", "2018-03-17T10:23:09+00:00", 75, &fakeClient)
				Expect(err).NotTo(HaveOccurred())
				Expect(fakeClient.CreateStatsCallCount()).To(Equal(1))
		...
		Context("When no date is provided", func() {
			...
			It("Uses the current time", func() {
				...
				err := commands.CreateStat("a-resource", "key1|key2", "", 75, &fakeClient)
				Expect(err).NotTo(HaveOccurred())
				Expect(fakeClient.CreateStatsCallCount()).To(Equal(1))
		...
		Context("When an invalid date", func() {
			It("Returns an error", func() {
				err := commands.CreateStat("a-resource", "key1|key2", "not-a-date", 75, &fakeClient)
		...
		Context("When the client returns an error", func() {
			It("Returns the error back to the caller", func() {
				...
				err := commands.CreateStat("a-resource", "key1|key2", "12345", 75, &fakeClient)
